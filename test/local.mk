...
local-install: kind-load-image crossplane-setup registry-setup minio-setup mirror-setup package-push-local ## Install Operator in local cluster
	kubectl apply -f test/controllerconfig-s3.yaml
	yq e '.spec.package="${INTERNAL_PACKAGE_IMG}"' test/provider-s3.yaml | kubectl apply -f -
	kubectl wait --for condition=Healthy provider.pkg.crossplane.io/provider-s3 --timeout 60s
	kubectl -n crossplane-system wait --for condition=Ready $$(kubectl -n crossplane-system get pods -o name -l pkg.crossplane.io/provider=provider-s3) --timeout 60s
...
$(kind_dir)/.credentials.yaml:
	kubectl create secret generic --from-literal s3_API_KEY=minioadmin --from-literal s3_API_SECRET=minioadmin -o yaml --dry-run=client api-secret > $@
...
provider-config: $(KIND_KUBECONFIG) $(kind_dir)/.credentials.yaml
	kubectl apply -n crossplane-system -f $(kind_dir)/.credentials.yaml -f samples/s3.crossplane.io_providerconfig.yaml
...
minio-setup: kind-setup-ingress ## Install minio Crossplane implementation
	...
	@echo -e "***\n*** Installed s3 in http://minio.127.0.0.1.nip.io:8088\n***"
