...
spec:
  ...
  endpoint: http://minio-server.minio.svc:9000/ <2>
status: {}

----
<1> Please adjust the credendials according to your s3 installation
<2> Please adjust the URL according to your s3 installation
...
. Wait until the operator has provisioned the bucket
+
[source,bash]
----
