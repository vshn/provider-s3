...
install-samples: ## Install samples into cluster
	...
	yq ./samples/s3*.yaml | kubectl apply -f -
...
webhook-debug:
	...
	yq e "del(.webhooks[0].clientConfig.service) | .webhooks[0].clientConfig.caBundle |= \"$$cabundle\" | .webhooks[0].clientConfig.url |= \"https://$$HOSTIP:9443//validate-s3-crossplane-io-v1-bucket\"" - | \
	yq e "del(.webhooks[1].clientConfig.service) | .webhooks[1].clientConfig.caBundle |= \"$$cabundle\" | .webhooks[1].clientConfig.url |= \"https://$$HOSTIP:9443//validate-s3-crossplane-io-v1-policy\"" - | \
	yq e "del(.webhooks[2].clientConfig.service) | .webhooks[2].clientConfig.caBundle |= \"$$cabundle\" | .webhooks[2].clientConfig.url |= \"https://$$HOSTIP:9443//validate-s3-crossplane-io-v1-user\"" - | \
	kubectl apply -f - && \
