...
func TestProvisioningPipeline_Observe(t *testing.T) {
	...
	tests := map[string]struct {
		...
		expectedBucketObservation s3v1.BucketProviderStatus
	}{
		"NewBucketDoesntYetExistOnS3": {
			givenBucket: &s3v1.Bucket{Spec: s3v1.BucketSpec{ForProvider: s3v1.BucketParameters{
				BucketName: "my-bucket"}},
			},
			expectedResult: managed.ExternalObservation{},
		},
		"NewBucketWithPolicyDoesntYetExistOnS3": {
			givenBucket: &s3v1.Bucket{Spec: s3v1.BucketSpec{ForProvider: s3v1.BucketParameters{
		...
		"BucketExistsAndAccessibleWithOurCredentials": {
			givenBucket: &s3v1.Bucket{
				...
				Spec: s3v1.BucketSpec{ForProvider: s3v1.BucketParameters{
					BucketName: "my-bucket"}},
			},
			bucketExists:              true,
			expectedResult:            managed.ExternalObservation{ResourceExists: true, ResourceUpToDate: true},
			expectedBucketObservation: s3v1.BucketProviderStatus{BucketName: "my-bucket"},
		},
		...
		"BucketAlreadyExistsOnS3_WithoutAccess": {
			givenBucket: &s3v1.Bucket{Spec: s3v1.BucketSpec{ForProvider: s3v1.BucketParameters{
				BucketName: "my-bucket"}},
			},
		...
		"BucketAlreadyExistsOnS3_WithAccess_PreventAdoption": {
		...
		"BucketAlreadyExistsOnS3_InAnotherZone": {
			givenBucket: &s3v1.Bucket{
				Spec: s3v1.BucketSpec{ForProvider: s3v1.BucketParameters{
					BucketName: "my-bucket"}},
			},
